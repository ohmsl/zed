                                ...
                                .children(
                                    linked_worktrees.into_iter().map(|wt| {
                                        let worktree_label = wt.worktree_name.clone().map(|name| {
                                            // If the worktree name contains a colon (used to
                                            // disambiguate worktrees that share a name), style the
                                            // colon like the slash separator so the prefix reads
                                            // as a qualifier rather than part of the name.
                                            let (prefix, rest, rest_positions) =
                                                if let Some(colon_byte) = name.find(':') {
                                                    let prefix = SharedString::from(
                                                        name[..colon_byte].to_string(),
                                                    );
                                                    let rest = SharedString::from(
                                                        name[colon_byte + 1..].to_string(),
                                                    );
                                                    ...
                                                    (None, name, wt.highlight_positions.clone())
                                                };

                                            let rest_label = if rest_positions.is_empty() {
                                                Label::new(rest)
                                                    .size(LabelSize::Small)
                                                    .color(Color::Muted)
                                                    .truncate()
                                                ...
                                                HighlightedLabel::new(rest, rest_positions)
                                                    .size(LabelSize::Small)
                                                    .color(Color::Muted)
                                                    .truncate()
                                                    .into_any_element()
